/**
 * 公历农历互转（1900-01-31 至 2100-12-31）
 * 算法与数据表前端硬编码，不调用任何网络或黄历接口。
 */

export interface LunarDate {
  year: number
  month: number
  day: number
  isLeap: boolean
}

export interface SolarDate {
  year: number
  month: number
  day: number
}

const LUNAR_START_YEAR = 1900
const LUNAR_END_YEAR = 2100

/**
 * 1900-2100 农历信息。低 4 位闰月月份，中间 12 位正月到腊月大小月，高位闰月天数。
 */
const LUNAR_INFO = [
  0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2,
  0x04ae0, 0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977,
  0x04970, 0x0a4b0, 0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970,
  0x06566, 0x0d4a0, 0x0ea50, 0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950,
  0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0, 0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950, 0x0b557,
  0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5d0, 0x14573, 0x052d0, 0x0a9a8, 0x0e950, 0x06aa0,
  0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260, 0x0f263, 0x0d950, 0x05b57, 0x056a0,
  0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558, 0x0b540, 0x0b6a0, 0x195a6,
  0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46, 0x0ab60, 0x09570,
  0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x055c0, 0x0ab60, 0x096d5, 0x092e0,
  0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5,
  0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930,
  0x07954, 0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530,
  0x05aa0, 0x076a3, 0x096d0, 0x04bd7, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45,
  0x0b5a0, 0x056d0, 0x055b2, 0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0,
  0x14b63, 0x09370, 0x049f8, 0x04970, 0x064b0, 0x168a6, 0x0ea50, 0x06aa0, 0x1a6c4, 0x0aae0,
  0x092e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0, 0x0da50, 0x05d55, 0x056a0, 0x0a6d0, 0x055d4,
  0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0, 0x0b6a6, 0x0ad50, 0x055a0, 0x0aba4, 0x0a5b0, 0x052b0,
  0x0b273, 0x06930, 0x07337, 0x06aa0, 0x0ad50, 0x14b55, 0x04b60, 0x0a570, 0x054e4, 0x0d160,
  0x0e968, 0x0d520, 0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a4d0, 0x0d150, 0x0f252,
  0x0d520
]

const MONTH_NAMES = ['正月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月']
const DAY_NAMES = [
  '初一', '初二', '初三', '初四', '初五', '初六', '初七', '初八', '初九', '初十',
  '十一', '十二', '十三', '十四', '十五', '十六', '十七', '十八', '十九', '二十',
  '廿一', '廿二', '廿三', '廿四', '廿五', '廿六', '廿七', '廿八', '廿九', '三十'
]

function yearInfo(year: number): number {
  return LUNAR_INFO[year - LUNAR_START_YEAR]
}

export function isSupportedYear(year: number): boolean {
  return year >= LUNAR_START_YEAR && year <= LUNAR_END_YEAR
}

export function leapMonth(year: number): number {
  if (!isSupportedYear(year)) return 0
  return yearInfo(year) & 0xf
}

export function leapDays(year: number): number {
  if (leapMonth(year) === 0) return 0
  return yearInfo(year) & 0x10000 ? 30 : 29
}

export function monthDays(year: number, month: number): number {
  if (!isSupportedYear(year) || month < 1 || month > 12) return 0
  return yearInfo(year) & (0x10000 >> month) ? 30 : 29
}

export function yearDays(year: number): number {
  let total = 348
  for (let i = 0x8000; i > 0x8; i >>= 1) {
    total += yearInfo(year) & i ? 1 : 0
  }
  return total + leapDays(year)
}

function utcDays(year: number, month: number, day: number): number {
  return Date.UTC(year, month - 1, day) / 86400000
}

/** 农历 1900 正月初一对应公历 1900-01-31 */
const BASE_SOLAR_DAYS = utcDays(1900, 1, 31)

export function solarToLunar(year: number, month: number, day: number): LunarDate {
  if (year < 1900 || year > 2100) {
    throw new Error('仅支持 1900-2100 年')
  }
  const offset = utcDays(year, month, day) - BASE_SOLAR_DAYS
  if (offset < 0) {
    throw new Error('仅支持 1900-01-31 之后的日期')
  }

  let remain = offset
  let lunarYear = LUNAR_START_YEAR
  for (; lunarYear <= LUNAR_END_YEAR; lunarYear += 1) {
    const days = yearDays(lunarYear)
    if (remain < days) break
    remain -= days
  }

  const leap = leapMonth(lunarYear)
  let lunarMonth = 1
  let isLeap = false

  for (lunarMonth = 1; lunarMonth <= 12; lunarMonth += 1) {
    if (leap > 0 && lunarMonth === leap + 1 && !isLeap) {
      lunarMonth -= 1
      isLeap = true
      const days = leapDays(lunarYear)
      if (remain < days) break
      remain -= days
      isLeap = false
      continue
    }
    const days = monthDays(lunarYear, lunarMonth)
    if (remain < days) break
    remain -= days
  }

  return {
    year: lunarYear,
    month: lunarMonth,
    day: remain + 1,
    isLeap
  }
}

export function lunarToSolar(year: number, month: number, day: number, isLeap = false): SolarDate {
  if (!isSupportedYear(year) || month < 1 || month > 12) {
    throw new Error('农历日期超出范围')
  }
  const maxDay = isLeap ? leapDays(year) : monthDays(year, month)
  if (isLeap && leapMonth(year) !== month) {
    throw new Error('该年没有对应闰月')
  }
  if (day < 1 || day > maxDay) {
    throw new Error('农历日期无效')
  }

  let offset = 0
  for (let y = LUNAR_START_YEAR; y < year; y += 1) {
    offset += yearDays(y)
  }

  const leap = leapMonth(year)
  for (let m = 1; m < month; m += 1) {
    offset += monthDays(year, m)
    if (leap === m) offset += leapDays(year)
  }
  if (isLeap) {
    offset += monthDays(year, month)
  }
  offset += day - 1

  const date = new Date(Date.UTC(1900, 0, 31) + offset * 86400000)
  return {
    year: date.getUTCFullYear(),
    month: date.getUTCMonth() + 1,
    day: date.getUTCDate()
  }
}

export function lunarMonthLabel(month: number, isLeap = false): string {
  const name = MONTH_NAMES[month - 1] || `${month}月`
  return isLeap ? `闰${name}` : name
}

export function lunarDayLabel(day: number): string {
  return DAY_NAMES[day - 1] || `${day}日`
}

export function formatLunar(date: LunarDate, withYear = true): string {
  const month = lunarMonthLabel(date.month, date.isLeap)
  const day = lunarDayLabel(date.day)
  return withYear ? `${date.year}年${month}${day}` : `${month}${day}`
}

export function getLunarMonthOptions(year: number): Array<{ month: number; isLeap: boolean; label: string }> {
  const options: Array<{ month: number; isLeap: boolean; label: string }> = []
  const leap = leapMonth(year)
  for (let month = 1; month <= 12; month += 1) {
    options.push({ month, isLeap: false, label: lunarMonthLabel(month) })
    if (leap === month) {
      options.push({ month, isLeap: true, label: lunarMonthLabel(month, true) })
    }
  }
  return options
}

export function getLunarDayCount(year: number, month: number, isLeap: boolean): number {
  return isLeap ? leapDays(year) : monthDays(year, month)
}
